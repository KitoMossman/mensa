...
- **Teilnehmer stellt Anfrage (`index.php`):** Im Kontaktformular existiert bei Abgabe die Option *"Ich möchte eine Antwort der Küche (ein Abruf-Code wird generiert)"*. Nach dem Absenden generiert die Applikation automatisch eine alphanumerische **Ticket-ID** (z.B. **4BF72A**) sowie auslesbare, anwenderfreundliche Zufallspasswörter (ein echtes Wort plus Nummern, z.B. *"Tomate82"*). 
- **Küche antwortet (`nachrichten.php`):** In der Admin-Ansicht wird die Nachricht visuell vom Rest hervorgehoben (`w3-badge`). Die Küche findet direkt dort ein vorgefertigtes Antwortfeld für diese Nachricht. *(Hinweis: Das Küchen-Personal sieht das "Geheimwort" gar nicht erst, da es kryptografisch verschlüsselt gespeichert wurde).*
- **Teilnehmer liest (`abrufen.php`):** Über eine völlig eigenständige Seite können Teilnehmer künftig ihre *Ticket-ID* und ihr Geheimwort eintragen. Das Profil der Frage sowie die (wenn bereits gestellte) Antwort von der Küche sind daraufhin exklusiv ersichtlich!
