...
?>
  ...
  <header class="hero-header w3-center">
    <h1>Posteingang</h1>
    <p class="w3-text-muted">Hier finden Sie alle Nachrichten und Rückmeldungen.</p>
  </header>
    ...
    <div class="modern-card">
      <div class="w3-responsive">
        <table class="modern-table">
          <tr>
            <th style="width:15%">Status / Zeit</th>
          ...
          while ($row = $stmt->fetch()) {
              ...
              if (!empty($row['erstellt_am'])) {
                  echo "<div class='w3-small'><b>Erstellt:</b><br>" . h((new DateTime($row['erstellt_am']))->format('d.m.y H:i')) . "</div><br>";
              } else {
              ...
              if ($row['antwort_gewuenscht']) {
                  if (!empty($row['abgerufen_am'])) {
                      echo "<div class='w3-small w3-text-green'><b>Gelesen:</b><br>" . h((new DateTime($row['abgerufen_am']))->format('d.m.y H:i')) . "</div><br>";
                      ...
                      echo "<button type='submit' class='modern-btn secondary small'><i class='fa fa-trash'></i> Löschen</button>";
                      echo "</form>";
                  } else {
              ...
              if ($row['antwort_gewuenscht']) {
                  echo "<br><span class='w3-badge w3-blue w3-tiny' style='margin-top:5px;'>ID: " . h($row['ticket_id']) . "</span>";
              }
              echo "</td>";
              
              // Nachricht
              echo "<td><div style='white-space: pre-wrap; word-break: break-all; font-size:14px; opacity: 0.9;'>" . h($row['nachricht']) . "</div></td>";
              ...
              if ($row['antwort_gewuenscht']) {
                  ...
                  echo "<textarea name='antwort_text' class='w3-input w3-border w3-round w3-transparent w3-text-white w3-margin-bottom' rows='3' placeholder='Antwort verfassen...'>" . h($row['antwort']) . "</textarea>";
                  if (empty($row['antwort'])) {
                      echo "<button type='submit' class='modern-btn small' style='width:100%'><i class='fa fa-reply'></i> Antworten</button>";
                  } else {
                      echo "<button type='submit' class='modern-btn small' style='width:100%; background: #f59e0b;'><i class='fa fa-pencil'></i> Bearbeiten</button>";
                  }
                  echo "</form>";
  
                  // Anzeige Rückantwort Nutzer
                  if (!empty($row['nutzer_rueckantwort'])) {
                      echo "<div class='w3-margin-top w3-padding w3-round w3-small' style='background:rgba(59,130,246,0.1); border:1px solid rgba(59,130,246,0.2);'>";
                      echo "<b>Rückantwort:</b><br>";
                      echo "<span style='white-space: pre-wrap;'>" . h($row['nutzer_rueckantwort']) . "</span>";
                      echo "</div>";
                  }
              } else {
                  echo "<span class='w3-text-muted w3-small'>Keine Antwort angefordert.</span>";
